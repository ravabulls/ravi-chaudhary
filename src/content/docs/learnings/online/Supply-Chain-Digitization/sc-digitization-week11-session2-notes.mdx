...
<Aside type="note">
The four demand markets — **Ahmedabad, Surat, Mumbai and Pune** — shown on a map of western India. Greenfield Analysis (GFA) searches this open geography for the single DC location that minimises total distance-weighted demand — a "greenfield" site unconstrained by any existing facility.
</Aside>
...
<figure class="sc-fig">
  <div class="sc-formula">
    <div>
      <div class="name">Haversine (great-circle) distance</div>
      <code>a = sin²(Δlat/2) + cos(lat₁)·cos(lat₂)·sin²(Δlon/2)<br/>c = 2·atan2(√a, √(1−a))<br/>distance = R·c &nbsp;(R = earth's radius = 6371 km)</code>
    </div>
    <div>
      <div class="name">In our context</div>
      <code>Δlat = x_DC − xᵢ &nbsp;·&nbsp; Δlon = y_DC − yᵢ<br/>distance((xᵢ, yᵢ), (x_DC, y_DC)) = R·c</code>
    </div>
  </div>
  <figcaption>The Haversine formula gives the true over-the-earth distance between two latitude/longitude points — used to measure each market's distance from a candidate DC location.</figcaption>
</figure>
...
*Excel setup — with a trial DC at (20.00, 72.00), the Haversine distance to each market and the total distance-weighted cost:*

| Market | Cost/km·unit | Demand (units) | Latitude | Longitude | Distance (km) |
|---|---|---|---|---|---|
...
| **Trial DC (20.00, 72.00)** | | | | | **Cost = 124,489,119** |
