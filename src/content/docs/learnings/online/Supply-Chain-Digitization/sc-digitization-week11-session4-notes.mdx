...
*Demand data — four markets (yearly demand doubled for the 3-year horizon):*

| Market | Yearly Demand | Latitude | Longitude |
|---|---|---|---|
...
*Candidate factory data:*

| Factory | Lat | Long | Fixed cost/yr ($) | Prod. cost/unit ($) | → Vapi (km) | → Bhiwandi (km) |
|---|---|---|---|---|---|---|
...
| DC | Lat | Long | Fixed cost/yr ($) | Inbound/unit | Outbound/unit |
|---|---|---|---|---|---|
| **Vapi** | 20.42 | 72.91 | 1,825,000 | 0.25 | 0.25 |
| **Bhiwandi** | 19.29 | 73.05 | 2,737,500 | 0.50 | 0.50 |

| DC → market distance (km) | Mumbai | Pune | Surat | Ahmedabad |
|---|---|---|---|---|
...
<figure class="sc-fig">
  <div class="sc-formula">
    <div>
      <div class="name">Objective — maximise profit</div>
      <code>max &nbsp;Σ Rev·Q<sub>jk</sub> − [ Σ FixedFac<sub>i</sub>·Y<sub>i</sub> + Σ FixedDC<sub>j</sub>·Z<sub>j</sub> + Σ ProdFac<sub>i</sub>·X<sub>ij</sub> + Σ InbDC<sub>j</sub>·X<sub>ij</sub> + Σ OutbDC<sub>j</sub>·Q<sub>jk</sub> + Σ Trans<sub>ij</sub>·X<sub>ij</sub> + Σ Trans<sub>jk</sub>·Q<sub>jk</sub> ]</code>
    </div>
    <div>
      <div class="name">Constraints</div>
      <code>Σ<sub>j</sub> Q<sub>jk</sub> = demand<sub>k</sub> &nbsp;·&nbsp; Σ<sub>j</sub> X<sub>ij</sub> ≤ M·Y<sub>i</sub> &nbsp;·&nbsp; Σ<sub>i</sub> X<sub>ij</sub> ≤ M·Z<sub>j</sub><br/>Σ<sub>i</sub> X<sub>ij</sub> = Σ<sub>k</sub> Q<sub>jk</sub> (flow balance) &nbsp;·&nbsp; Y<sub>i</sub>, Z<sub>j</sub> ∈ {0,1} &nbsp;·&nbsp; X<sub>ij</sub>, Q<sub>jk</sub> ≥ 0</code>
    </div>
  </div>
  <figcaption>The full network-optimisation model — binary variables Y<sub>i</sub> / Z<sub>j</sub> decide which factories and DCs to open, while X<sub>ij</sub> / Q<sub>jk</sub> set the flows, all chosen to maximise revenue minus fixed, production, handling and transport costs.</figcaption>
</figure>
...
<figure class="sc-fig">
  <div class="sc-flow">
    <div class="sc-tier"><div class="sc-tier-head">Factories</div><div class="sc-node amber">Aurangabad</div><div class="sc-node amber">Nashik</div></div>
    <span class="arrow">→</span>
    <div class="sc-tier"><div class="sc-tier-head">Distribution centres</div><div class="sc-node">Bhiwandi</div><div class="sc-node">Vapi</div></div>
    <span class="arrow">→</span>
    <div class="sc-tier"><div class="sc-tier-head">Customers</div><div class="sc-node violet">Mumbai</div><div class="sc-node violet">Pune</div><div class="sc-node violet">Surat</div><div class="sc-node violet">Ahmedabad</div></div>
  </div>
  <figcaption>The optimised network — the model selects which factories and DCs to open and routes each market's demand through the least-cost factory→DC→customer path, the result a tool like anyLogistix also plots on a map.</figcaption>
</figure>
...
<Aside type="note">
Drilling into a single flow (e.g. **Aurangabad → Vapi**) shows the per-link quantity and cost the optimiser assigned — the granular routing detail behind the network-level result above.
</Aside>
