...
<figure class="sc-fig">
  <table class="sc-matrix">
    <thead>
      <tr><th>SC type ↓ / Product →</th><th>Functional products</th><th>Innovative products</th></tr>
    </thead>
    <tbody>
      <tr><th>Efficient SC (Push)</th><td class="hl">Match</td><td>Mismatch</td></tr>
      <tr><th>Responsive SC (Pull)</th><td>Mismatch</td><td class="hl">Match</td></tr>
    </tbody>
  </table>
  <figcaption>Match the product to the supply chain: functional products fit an efficient/push chain, innovative products fit a responsive/pull chain. The off-diagonal combinations are structural mismatches.</figcaption>
</figure>
...
<figure class="sc-fig">
  <div class="sc-flow">
    <div class="sc-node soft">Suppliers</div>
    <span class="arrow">→</span>
    <div class="sc-node">Push zone<span class="sub">forecast-driven · low uncertainty</span></div>
    <span class="sc-bnd">Push–Pull<br/>boundary</span>
    <div class="sc-node violet">Pull zone<span class="sub">demand-driven · high uncertainty</span></div>
    <span class="arrow">→</span>
    <div class="sc-node soft">Customers</div>
  </div>
  <figcaption>Most real supply chains are hybrids: an upstream push zone (built to forecast) and a downstream pull zone (triggered by real demand), separated by the push–pull boundary.</figcaption>
</figure>
...
<figure class="sc-fig">
  <div class="sc-flow">
    <div class="sc-node soft">Suppliers</div>
    <span class="arrow">→</span>
    <div class="sc-node">Manufacturing<span class="sub">assembly lines</span></div>
    <span class="arrow">→</span>
    <div class="sc-node slate">Distributors</div>
    <span class="sc-bnd">Push–Pull<br/>boundary</span>
    <div class="sc-node violet">Customers</div>
  </div>
  <div class="sc-legend">
    <span><i style="background:var(--sc-blue)"></i>Push — standard vehicle, forecast-driven (Suppliers → Distributor)</span>
    <span><i style="background:var(--sc-violet)"></i>Pull — colour &amp; accessory customisation (Distributor → Customer)</span>
  </div>
  <figcaption>Example 1 — standardise upstream with push, then customise downstream with pull. The boundary sits at the distributor: the last player before the customer's specific choices enter the chain.</figcaption>
</figure>
...
<figure class="sc-fig">
  <div class="sc-flow">
    <div class="sc-node violet">Suppliers</div>
    <span class="sc-bnd">Push–Pull<br/>boundary</span>
    <div class="sc-node">Manufacturing<span class="sub">assembly lines</span></div>
    <span class="arrow">→</span>
    <div class="sc-node slate">Distributors</div>
  ...
  <div class="sc-legend">
    <span><i style="background:var(--sc-violet)"></i>Pull — suppliers deliver components per order (Suppliers)</span>
    <span><i style="background:var(--sc-blue)"></i>Push — standard assembly &amp; delivery (Manufacturing → Customer)</span>
  </div>
  <figcaption>Example 2 — when customisation happens at the component level, the pull zone extends all the way back to suppliers; once assembly is standard, the chain reverts to push. The boundary sits between supplier and manufacturing.</figcaption>
</figure>
