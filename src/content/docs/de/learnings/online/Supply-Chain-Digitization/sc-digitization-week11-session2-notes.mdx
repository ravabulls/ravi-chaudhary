...
<Aside type="note">
Die vier Nachfragemärkte — **Ahmedabad, Surat, Mumbai und Pune** — auf einer Karte Westindiens. Die Greenfield-Analyse (GFA) durchsucht diese offene Geografie nach dem einen DC-Standort, der die gesamte entfernungsgewichtete Nachfrage minimiert — ein „grüner" Standort, ungebunden an jede bestehende Anlage.
</Aside>
...
<figure class="sc-fig">
  <div class="sc-formula">
    <div>
      <div class="name">Haversine-Distanz (Großkreis)</div>
      <code>a = sin²(Δlat/2) + cos(lat₁)·cos(lat₂)·sin²(Δlon/2)<br/>c = 2·atan2(√a, √(1−a))<br/>distance = R·c &nbsp;(R = Erdradius = 6371 km)</code>
    </div>
    <div>
      <div class="name">In unserem Kontext</div>
      <code>Δlat = x_DC − xᵢ &nbsp;·&nbsp; Δlon = y_DC − yᵢ<br/>distance((xᵢ, yᵢ), (x_DC, y_DC)) = R·c</code>
    </div>
  </div>
  <figcaption>Die Haversine-Formel liefert die tatsächliche Entfernung über die Erdoberfläche zwischen zwei Breiten-/Längengrad-Punkten — hier zur Messung der Entfernung jedes Marktes von einem möglichen DC-Standort.</figcaption>
</figure>
