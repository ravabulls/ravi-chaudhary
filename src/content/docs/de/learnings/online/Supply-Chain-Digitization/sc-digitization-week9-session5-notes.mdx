...
*Neun Anlagen - zwei Outputs (höher = besser) und zwei Inputs (niedriger = besser):*

| Anlage | Produktionsausbeute (%) | OEE (%) | Taktzeit (Min.) | Ressourcennutzung (%) |
|---|---|---|---|---|
| 1 | 92 | 80 | 32 | 88 |
| 2 | 88 | 78 | 35 | 94 |
| 3 | 92 | 81 | 31 | 95 |
...
<figure class="sc-fig">
  <div class="sc-formula">
    <div>
      <div class="name">Zielfunktion (für Anlage 1, nach Linearisierung)</div>
      <code>max &nbsp;92·v<sub>1</sub> + 80·v<sub>2</sub></code>
    </div>
    <div>
      <div class="name">Nebenbedingungen</div>
      <code>32·u<sub>1</sub> + 88·u<sub>2</sub> = 1 &nbsp;(Input-Gewichte normiert)<br/>∀ Anlage: (Ausbeute·v<sub>1</sub> + OEE·v<sub>2</sub>) − (Taktzeit·u<sub>1</sub> + Nutzung·u<sub>2</sub>) ≤ 0</code>
    </div>
  </div>
  <figcaption>DEA wird linearisiert, indem die gewichtete Input-Summe auf 1 fixiert und der gewichtete Output der bewerteten Anlage maximiert wird. v<sub>k</sub> gewichten die Outputs, u<sub>j</sub> die Inputs; die ≤-0-Bedingung hält keine Anlage über der Effizienzgrenze.</figcaption>
</figure>
